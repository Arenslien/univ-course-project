...
class DeepCF(nn.Module):
    ...
    def encode(self, Uin, Vin, Ucontent, Vcontent):
        ...
        # User feature 체크 후 병합(concat)
        if self.phi_u_dim>0:
            Ucontent = torch.tensor(Ucontent, dtype=torch.float32)
            u_concat = torch.cat((Uin, Ucontent), 1)
        else:
            u_concat = torch.tensor(Uin, dtype=torch.float32)

        # Item feature 체크 후 병합(concat)
        if self.phi_v_dim>0:
            Vcontent = torch.tensor(Vcontent, dtype=torch.float32)
    ...
    def get_all_rating_matrix(self, all_data, device=None, batch_size=100):
        """
        given ALL Data runs batch evaluation
        :param all_data: AllData instance
        :return: all_rating_matrix
        """
        d = device
        ...
        rating_matrices = []
        for (batch, (eval_start, eval_stop)) in enumerate(tqdm(all_data.eval_batch, desc='eval', leave=False)):

            Uin = all_data.U_pref_test[eval_start:eval_stop, :]
            Vin = all_data.V_pref_test
            Vcontent = all_data.V_content_test

            if self.phi_u_dim > 0: 
                Ucontent= all_data.U_content_test[eval_start:eval_stop, :]
            else:
                Ucontent = None
            ...
            rating_matrices.append(embedding_prod.detach().cpu().numpy())
            all_U_embedding.append(U_embedding.detach().cpu().numpy())
        
        all_V_embedding = V_embedding.detach().cpu().numpy()
        
        all_rating_matrix = np.concatenate(rating_matrices, axis=0)
        all_U_embedding = np.array(np.concatenate(all_U_embedding, axis=0))
        all_V_embedding = np.array(all_V_embedding)
        
        return all_rating_matrix, all_U_embedding, all_V_embedding
    
    def get_embedding(self, Uin, Vin, Ucontent, Vcontent):
