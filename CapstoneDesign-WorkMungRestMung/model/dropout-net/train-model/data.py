...
def load_eval_data(test_file, test_id_file, name, cold, train_data, citeu=False):
    ...
    with open(test_id_file) as f:
        ...
        test_data_df = pd.read_csv(test_file, delimiter=",", header=None, dtype=np.int32)
        test_data = test_data_df.values.ravel()
...
class EvalData:
    ...
    def __init__(self, test_triplets, test_item_ids, is_cold, train):
        ...
        if train is not None:
            train_ij_for_inf = [(self.test_user_ids_map[_t[0]], self.test_item_ids_map[_t[1]]) for _t in train if _t[1] in self.test_item_ids_map and _t[0] in self.test_user_ids_map]
        else:
            train_ij_for_inf = []
    ...
    def init_tf(self, user_factors, item_factors, user_content, item_content, eval_run_batchsize):
        ...
        # 기존 코드는 item_content를 기본으로 받기 때문에 None 체크 조건문이 없음, 추가한 코드
